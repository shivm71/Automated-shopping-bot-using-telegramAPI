...
from src import telegram_connect as tc
...
async def send_msg_with_username():
    ...
    colllection = mc.db["user_with_uname"]   ## change according to db collection name
    curr_index = 0
    for user in colllection.find({"invite_status": False}):  ##todo: need to implement limit offset logic, & get object in sorted order of the created time
        ...
        try:
            ...
            database_operations.update_invite_status(colllection,user["_id"])
