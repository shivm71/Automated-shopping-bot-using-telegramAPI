...
def intercall(client_name,client,group_id):
    loop.run_until_complete(insert(client_name,client,group_id))


async def insert(client_name,client,group_id):
    count = 0
    ...
    try:
        async for user in client.iter_participants(entity,limit=500):   #aggressive = True
            if database_operations.insert_user(client_name,user):
                count+=1
            else:
...
for client_name in client_list:
    x = threading.Thread(target= intercall, args=(client_name,client_dict[client_name],'campusdriveupdates'))
    x.start()
    x.join()
